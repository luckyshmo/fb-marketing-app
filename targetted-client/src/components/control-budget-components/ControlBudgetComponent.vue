  ...
  <div>
    <div class="control__budget_block_left">
      <div class="control__budget_range">
        <div>
          <h4>Бюджет в день <span>:</span> <span>{{ budgetPerDay }} ₽</span></h4>
          <vue-range-slider
            v-model="budgetPerDay"
        ...
        <div>
          <h4>Длительность <span>:</span>  <span>ещё {{ duration }} дня</span></h4>
          <vue-range-slider
            v-model="duration"
            height="8"
            :process-style="processStyle"
      ...
      <div class="control__budget_text">
        <div>
          <h2>{{ willSeeMin }}  – {{ willSeeMax }} человек</h2>
          <p>Увидят вашу рекламу</p>
        </div>
        <div>
          <h2>{{ willGoMin }}  – {{ willGoMax }} человек</h2>
          <p>Перейдут по вашей рекламе</p>
        </div>
        <div>
          <h2>{{ totalCostsMoney }} ₽  за {{ totalCostsDay }} дней</h2>
          <p>Общие затраты</p>
        </div>
      </div>
    </div>
    <div class="control__budget_block_right">
      ...
      <h2>{{ campaignBalance }} ₽</h2>
      <div v-if="buttonBlock">
        <div></div>
        <p>{{ buttonBlockText }}</p>
